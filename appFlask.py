...
# 🛠 Executa os backups e sincronizações antes de conectar ao banco de dados
backup_manager.criar_estrutura_pastas()
backup_manager.sincronizar_arquivos()
backup_manager.verificar_data_e_fazer_backup()
...
def get_db_connection():
    try:
        caminho_main = backup_manager.caminho_main  # Obtém o caminho do banco principal
        conn_str = f"DRIVER={{Microsoft Access Driver (*.mdb, *.accdb)}};DBQ={caminho_main}"
...
def identificar_nomes(tokens):
    ...
    if not conn:
        return None
    cursor = conn.cursor()
    query = "SELECT Nome FROM Nome WHERE Nome IS NOT NULL"
...
def listar_nomes_disponiveis():
    try:
        ...
        if not conn:
            return
        query = "SELECT Nome.Nome FROM Nome WHERE Nome IS NOT NULL"
